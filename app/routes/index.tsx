...
const clampDistance = (value: number) =>
  Math.min(Math.max(Number.isFinite(value) ? value : DEFAULT_DISTANCE, DISTANCE_SLIDER.min), DISTANCE_SLIDER.max)
...
function IndexPage() {
  ...
  const searchDistance = React.useMemo(() => clampDistance(search.distance ?? DEFAULT_DISTANCE), [search.distance])
  ...
  const handleLegendToggle = (vehicle: VehicleKey) => {
    setActiveVehicles((prev) => {
      ...
      updateSearch({
        vehicles: normalized.length === vehicleKeys.length ? undefined : serializeVehicles(normalized),
      })
      return normalized
    })
  }
    ...
    <main className="flex flex-1 flex-col gap-8 px-6 py-10">
      ...
      <section className="mx-auto w-full max-w-7xl space-y-6 rounded-3xl border border-white/10 bg-white/5 p-6">
        <div className="space-y-3">
          ...
          <p className="text-sm text-slate-300">
            Each PKR/km figure bakes in local fuel and charging prices for the blend described below. Assumes petrol at
            PKR {ENERGY_INPUTS.petrolPricePerLitre.toLocaleString()} / L, home electricity at PKR
            {` ${ENERGY_INPUTS.homeElectricityPerKwh} / kWh`} and DC fast charging at PKR
            {` ${ENERGY_INPUTS.fastChargePerKwh} / kWh`} with a {homeSharePercent}% / {fastSharePercent}% home/DC split for
            electric miles.
          </p>
        </div>
        <div className="grid gap-4 md:grid-cols-3">
          {Object.values(DRIVE_PROFILES).map((driveProfile) => (
            <article key={driveProfile.key} className="rounded-2xl border border-white/10 bg-slate-900/40 p-4">
              <header className="space-y-1">
                <p className="text-xs uppercase tracking-wide text-slate-400">{driveProfile.label}</p>
              ...
              <dl className="mt-4 space-y-2 text-sm text-slate-200">
                {vehicleKeys.map((vehicle) => {
                  ...
                  return (
                    <div key={`${driveProfile.key}-${vehicle}`} className="flex items-center justify-between gap-3">
                      <dt className="text-xs uppercase tracking-wide text-slate-400">{vehicleMeta.label}</dt>
                      <dd className="text-right">
                        ...
                        <ul className="space-y-0.5 text-[0.7rem] text-slate-400">
                          {efficiencyEntries.map((entry) => (
                            <li key={`${vehicle}-${entry.label}`}>{entry.value} {entry.unit} · {entry.label}</li>
                          ))}
                        </ul>
                      </dd>
                    </div>
      ...
      <section className="mx-auto w-full max-w-7xl rounded-3xl border border-white/10 bg-white/5 p-6 text-xs text-slate-400">
        <p>
          ...
          tariffs and electricity slabs before making purchasing decisions.
        </p>
      </section>
    </main>
  )
